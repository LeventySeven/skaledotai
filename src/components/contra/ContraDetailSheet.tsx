...
interface ContraDetailSheetProps {
  ...
  onPatch: (id: string, data: Partial<ContraLead>) => void;
}
...
function useDebouncedField(externalValue: string, onCommit: (value: string) => void, delay = 500) {
...
function ContraDetailContent({ lead, onPatch }: { lead: ContraLead; onPatch: (id: string, data: Partial<ContraLead>) => void }) {
  const [theAsk, onTheAskChange] = useDebouncedField(lead.theAsk, (v) => onPatch(lead.id, { theAsk: v }));
  const [email, onEmailChange] = useDebouncedField(lead.email ?? "", (v) => onPatch(lead.id, { email: v || undefined }));
  const [notes, onNotesChange] = useDebouncedField(lead.notes ?? "", (v) => onPatch(lead.id, { notes: v || undefined }));
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Textarea
              ...
              rows={3}
              value={theAsk}
              onChange={(e) => onTheAskChange(e.target.value)}
            />
          </div>
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetPopup side="right">
        <ContraDetailContent lead={lead} onPatch={onPatch} />
      </SheetPopup>
    </Sheet>
  );
}
